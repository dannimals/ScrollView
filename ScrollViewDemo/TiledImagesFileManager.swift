...
class TiledImagesFileManager: NSObject {

    private let fileManager = FileManager.default
    private var tiledImagesPathName: String
    private var highResImagePathName: String
    private let imageID = "demo-image"
    private let currentUserID = "user-id"

    weak var delegate: TiledImagesFileManagerDelegate?

    private(set) var finishedDownloadingHighResImage = false

    var highResImage: CGImage? {
        guard
            let highResPathURL = highResPathURL, let dataProvider = CGDataProvider(url: highResPathURL as CFURL),
            let highResImage = CGImage(jpegDataProviderSource: dataProvider, decode: nil, shouldInterpolate: true, intent: .defaultIntent)
            else { return nil }
        return highResImage
    }
    ...
    init(tiledImagesPathName: String = "TiledImages", highResImagePathName: String = "HighResImage") {
    ...
    private lazy var dataCacheDirectory: URL? = {
        return TiledImageDemo.dataCacheDirectoryURL(forUserWithID: currentUserID)?.appendingPathComponent(tiledImagesPathName)
    }()
